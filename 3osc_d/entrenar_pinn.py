...
loss_total = 2·loss_clf + loss_reg + λ · loss_physics   (λ = 0.005)
"""
...
n_aire_tf = tf.complex(tf.ones(101), tf.zeros(101))
n_si_tf = tf.complex(
    tf.constant(np.real(n_si), dtype=tf.float32),
    tf.constant(np.imag(n_si), dtype=tf.float32))
...
X = df[col_tan + col_cos].values
Y_reg = df[col_params].values
Y_clf = df['n_osc'].values
...
def physics_loss_multi(Y_pred_real, X_batch_real):
    ...
    Parámetros:
        Y_pred_real  : (batch, 14) - parámetros TL desescalados
...
@tf.function
def train_step(X_batch, Y_clf_batch, Y_reg_batch):
    ...
    return loss_clf, loss_reg, loss_phys

# Separar validación (20%) del conjunto de entrenamiento
n_val = int(len(X_train_s) * 0.2)
idx_perm = np.random.RandomState(42).permutation(len(X_train_s))
val_idx, train_idx = idx_perm[:n_val], idx_perm[n_val:]

X_tr = tf.cast(X_train_s[train_idx], tf.float32)
Yreg_tr = tf.cast(Yreg_train_s[train_idx], tf.float32)
Yclf_tr = tf.cast(Yclf_train[train_idx], tf.int32)
X_val = tf.cast(X_train_s[val_idx], tf.float32)
Yreg_val = tf.cast(Yreg_train_s[val_idx], tf.float32)
Yclf_val = tf.cast(Yclf_train[val_idx], tf.int32)

dataset_train = tf.data.Dataset.from_tensor_slices(
    (X_tr, Yclf_tr, Yreg_tr)).shuffle(10000, seed=42).batch(32)
dataset_val = tf.data.Dataset.from_tensor_slices(
    (X_val, Yclf_val, Yreg_val)).batch(32)

hist = {'clf_tr':[], 'reg_tr':[], 'phys_tr':[],
        'clf_val':[], 'reg_val':[], 'phys_val':[]}
...
for epoch in range(300):
    ep_clf, ep_reg, ep_phys = [], [], []
    for X_b, Yclf_b, Yreg_b in dataset_train:
        lc, lr, lp = train_step(X_b, Yclf_b, Yreg_b)
        ep_clf.append(lc.numpy())
        ep_reg.append(lr.numpy())
        ep_phys.append(lp.numpy())

    ep_clf_v, ep_reg_v, ep_phys_v = [], [], []
    for X_b, Yclf_b, Yreg_b in dataset_val:
        y_clf_p, y_reg_p = model_pinn(X_b, training=False)

        lc = tf.reduce_mean(
            tf.keras.losses.sparse_categorical_crossentropy(Yclf_b, y_clf_p))
        lr = tf.reduce_mean(tf.square(Yreg_b - y_reg_p))
    ...
    m_clf = np.mean(ep_clf);     m_reg = np.mean(ep_reg);     m_phys = np.mean(ep_phys)
    m_clf_v = np.mean(ep_clf_v); m_reg_v = np.mean(ep_reg_v); m_phys_v = np.mean(ep_phys_v)

    hist['clf_tr'].append(m_clf);     hist['reg_tr'].append(m_reg);     hist['phys_tr'].append(m_phys)
    hist['clf_val'].append(m_clf_v);  hist['reg_val'].append(m_reg_v);  hist['phys_val'].append(m_phys_v)
