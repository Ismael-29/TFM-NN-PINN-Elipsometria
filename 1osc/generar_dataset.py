...
para una estructura aire / capa TL / Si, con parámetros TL aleatorios.
"""

import numpy as np
import pandas as pd
from modelo_tl import (eps_im, eps_re, indice_refraccion,
                        tmm_psi_delta, generar_parametros_tl)

# --- Configuración ---
MUESTRAS = 40000
THETA0 = 70 * np.pi / 180      # ángulo de incidencia
D_CAPA = 100                    # espesor fijo en nm
E = np.linspace(0.75, 3.65, 101)  # malla de energías (eV)
LAMBDA = 1239.8 / E               # conversión eV → nm (longitudes de onda)
...
# --- Generación de datos ---
# Para cada muestra:
#   1. Genera parámetros TL aleatorios (A, E₀, Eg, C, ε∞)
...
#      para la estructura: aire / capa TL (100 nm) / Si
#   5. Almacena los parámetros y los espectros como tan(Ψ) y cos(Δ)

resultados = []

for i in range(MUESTRAS):
    A, E_0, E_g, C, eps_inf = generar_parametros_tl()
    ...
    resultados.append([A, E_0, E_g, C, eps_inf] + list(tan_psi) + list(cos_delta))

# --- Crear DataFrame y guardar ---
# Columnas: 5 parámetros + 101 tan(Ψ) + 101 cos(Δ) = 207 columnas por muestra
...
df.to_csv('1osc/dataset_1osc.csv', index=False)
